...
func NewCompactSeriesFileCommand() *cobra.Command {
	...
	cmd.Flags().IntVar(&compactSeriesFileFlags.Concurrency, "concurrency", runtime.GOMAXPROCS(0), "Number of workers to dedicate to compaction. Defaults to GOMAXPROCS")
...
func RunCompactSeriesFile(cmd *cobra.Command, args []string) error {
	...
	for i := 0; i < compactSeriesFileFlags.Concurrency; i++ {
		g.Go(func() error {
